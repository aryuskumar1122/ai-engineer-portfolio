(function(){
  ...
  function scatterAround(center, count, baseRadius){
    const pts = [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      const r = baseRadius + (Math.random() * 6); 
      
      const x = Math.cos(theta) * Math.sin(phi) * r;
      const y = Math.cos(phi) * r;
      const z = Math.sin(theta) * Math.sin(phi) * r;
      
      pts.push(center.clone().add(new THREE.Vector3(x, y, z)));
    }
    return pts;
  }
  ...
  function addNode(id, opts){
    ...
    nodesById[id] = {
      id, type:opts.type, position:opts.position.clone(), mesh, glow, label, hit,
      baseGlowScale:opts.glowScale, baseGlowOpacity:opts.glowOpacity,
      baseLabelScaleVec: label.scale.clone(),
  ...
  function addEdge(fromId, toId, color){
    ...
    scene.add(pulse);
    edges.push({
      toId, line, start:from.position, end:to.position, pulse, 
      speed:0.15+Math.random()*0.15, phase:Math.random(), baseOpacity: 1 
    });
  }
  ...
  async function buildGraph(){
    ...
    const positions = scatterAround(projHub.position, DATA.projects.length, 7.5);
    ...
    DATA.projects.forEach((p)=>{
      ...
      node.label.scale.setScalar(0.001);
      node.hit.scale.setScalar(0.001);
  ...
  function setNodeHighlight(id, on){
    ...
    animateValue(200, (e)=>{
      if(node.type === 'project' && !projectsExpanded) return;
  ...
  function onPointerUp(e){
    ...
    if(activePointers.size===0){
      ...
      if(dragMoved<6 && e.target === canvas){
        ...
        const hits = raycaster.intersectObjects(clickableMeshes);
        if(hits.length) {
          activateNode(hits[0].object.userData.nodeId);
  ...
  function toggleProjects(show) {
    ...
    DATA.projects.forEach(p => {
      ...
      const startS = node.mesh.scale.x;
      const targetS = show ? 1 : 0.001;
      
      const startLineOp = edge ? edge.line.material.opacity : 0;
      const targetLineOp = show ? 0.35 : 0;
      
      const startPulseOp = edge ? edge.baseOpacity : 0;
      const targetPulseOp = show ? 1 : 0;
      
      animateValue(600, (e) => {
        const s = startS + (targetS - startS) * e;
        node.mesh.scale.setScalar(s);
        node.glow.scale.setScalar(node.baseGlowScale * s);
        node.label.scale.copy(node.baseLabelScaleVec).multiplyScalar(s);
        node.hit.scale.setScalar(s); 
        
        if(edge) {
          edge.line.material.opacity = startLineOp + (targetLineOp - startLineOp) * e;
          edge.baseOpacity = startPulseOp + (targetPulseOp - startPulseOp) * e;
        }
      });
    });
  }
  ...
  function activateNode(id){
    ...
    if(node.type==='hub'){
      ...
      if (projectsExpanded) toggleProjects(false);
      openPanel(id);
      return;
    }
  ...
  function tick(now){
    ...
    edges.forEach(e=>{
      ...
      e.pulse.material.opacity = (Math.sin(t*Math.PI)*0.9+0.1) * (e.baseOpacity !== undefined ? e.baseOpacity : 1);
    });
